...
int main(){
    ...
    std::shared_ptr<Shape> middle = std::make_shared<Cube>();
    middle->set_transform(translation(-0.5, 1, 0.5) * rotation_y(PI/1.5) * rotation_x(PI/1.5));
    ...
    std::shared_ptr<Shape> right = std::make_shared<Cube>();
    ...
    std::shared_ptr<Shape> left = std::make_shared<Cube>();
    ...
    Camera cam = Camera(2048, 2048, PI/3.0f);
    ...
    Canvas canvas = Canvas(cam.hsize, cam.vsize);
    {
        LOG_DURATION("Render");
